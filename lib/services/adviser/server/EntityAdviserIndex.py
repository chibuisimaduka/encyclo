...
class EntityAdviserIndex:
  ...
  def __init__(self):
    ...
    for predicate_attrs in utils.query_sql("SELECT id,entity_id,associated_entity_id FROM association_definitions"):
      self.predicates_by_entity[int(predicate_attrs[1])] += [int(predicate_attrs[0])]
      self.predicates_by_entity[int(predicate_attrs[2])] += [int(predicate_attrs[0])]
      ...
      for association_attrs in utils.query_sql("""SELECT entity_id,associated_entity_id FROM associations
        ...
        self.associations_by_predicate[int(predicate_attrs[0])][int(association_attrs[0])] += [int(association_attrs[1])]
        self.associations_by_predicate[int(predicate_attrs[0])][int(association_attrs[1])] += [int(association_attrs[0])]
  ...
  def get_suggestions(self, category_id, limit, offset, predicates = None):
    ...
    if predicates == None or len(predicates) == 0: # No filter
      ...
      for associated_values in self.associations_by_predicate[predicate_id]:
